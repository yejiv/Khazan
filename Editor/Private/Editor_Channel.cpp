...
void CEditor_Channel::Update_TransformationMatrix(const vector<CEditor_Bone*>& Bones, _float fCurrentTrackPosition, _uint* pCurrentKeyFrameIndex)
{
    ...
    if (m_isBlendPreAnimation)
    {
        ...
        //보정: 회전 정규화 + NaN 제거
        //if (XMVector4Equal(vRotation, XMVectorZero()))
        //    vRotation = XMQuaternionIdentity();
        //vRotation = XMQuaternionNormalize(vRotation);
    }

    //if (m_isBlendPreAnimation && !m_isRootBone)
    //{
    //    // 이전 애니메이션과의 보간 처리 
    //    vScale = XMVectorLerp(m_vPrevScale, vScale, m_fAnimationRatio);
    //    vRotation = XMQuaternionSlerp(m_vPrevRotQuat, vRotation, m_fAnimationRatio);
    ...
    //    //vRotation = XMQuaternionNormalize(vRotation);
    //}
    //else if (m_isBlendPreAnimation && m_isRootBone)
    //{
    //    //OutputDebugStringA("[ROOT BONE] Blending SKIPPED!\n");
    //    m_isBlendPreAnimation = false;
    //}
