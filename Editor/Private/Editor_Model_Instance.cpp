...
CEditor_Model_Instance::CEditor_Model_Instance(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
    : CComponent { pDevice ,pContext }
{
}


CEditor_Model_Instance::CEditor_Model_Instance(const CEditor_Model_Instance& Prototype)
...
HRESULT CEditor_Model_Instance::Initialize_Prototype(MODELTYPE eModelType, const _char* pModelFilePath, _fmatrix PreTransformMatrix, const CEditor_ModelMesh_Instance::INSTANCE_DESC* pDesc)
{
...
HRESULT CEditor_Model_Instance::Initialize_Clone(void* pArg)
{
    for (_uint i = 0; i < m_iNumMeshes; ++i)
    {
        CHECK_FAILED(m_Meshes[i]->Initialize_Clone(pArg), E_FAIL);
    }

    return S_OK;
}

HRESULT CEditor_Model_Instance::Render(_uint iMeshIndex)
{
...
HRESULT CEditor_Model_Instance::Bind_Materials(CShader* pShader, const _char* pConstantName, _uint iMeshIndex, _uint iTextureType, _uint iTextureIndex)
{
...
HRESULT CEditor_Model_Instance::Bind_BoneMatrices(CShader* pShader, const _char* pConstantName, _uint iMeshIndex)
{
    if (iMeshIndex >= m_iNumMeshes)
        return E_FAIL;
...
_bool CEditor_Model_Instance::Play_Animation(_float fTimeDelta)
{
    m_isFinished = false;
...
void CEditor_Model_Instance::Set_Animation(_uint iIndex, _bool isLoop)
{
...
void CEditor_Model_Instance::ExportModel()
{
    // 기본 경로 설정
...
void CEditor_Model_Instance::LoadModel(_wstring strModelName)
{
...
void CEditor_Model_Instance::Update_DAT_From_JSON()
{
    string strBasePath = "../../Client/Bin/Resources/Data/";
    string strModelFolder = strBasePath + m_Model_Data.strModelName + "/";
...
HRESULT CEditor_Model_Instance::Ready_Meshes(const CEditor_ModelMesh_Instance::INSTANCE_DESC* pDesc)
{
    /* 매쉬 몇 개인지 저장하기 */
    ...
    {
        CEditor_ModelMesh_Instance* pMesh = CEditor_ModelMesh_Instance::Create(m_pDevice, m_pContext, m_eModelType, m_pAIScene->mMeshes[i], m_Bones, XMLoadFloat4x4(&m_PreTransformMatrix), pDesc);
        if (pMesh == nullptr)
        {
...
HRESULT CEditor_Model_Instance::Ready_Materials()
{
...
HRESULT CEditor_Model_Instance::Ready_Bones(const aiNode* pAINode, _int iParentIndex)
{
...
HRESULT CEditor_Model_Instance::Ready_Animation()
{
...
_bool CEditor_Model_Instance::Export_AnimationJson(const string& strFilePath, const string& strFilePath2)
{
    /* 전체 애니메이션 JSON 저장*/
...
_bool CEditor_Model_Instance::Export_MaterialJson(const string& strFilePath)
{
...
void CEditor_Model_Instance::Export_Binary(const string& strFilePath)
{
...
string CEditor_Model_Instance::PostProcessJSON(const string& jsonStr)
{
...
string CEditor_Model_Instance::CompressArray(const string& arrayStr)
{
...
CEditor_Model_Instance* CEditor_Model_Instance::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, MODELTYPE eModelType, const _char* pModelFilePath, _fmatrix PreTransformMatrix, const CEditor_ModelMesh_Instance::INSTANCE_DESC* pDesc)
{
    CEditor_Model_Instance* pInstance = new CEditor_Model_Instance(pDevice, pContext);

    if (FAILED(pInstance->Initialize_Prototype(eModelType, pModelFilePath, PreTransformMatrix, pDesc)))
    {
        MSG_BOX(TEXT("Failed to Created : CEditor_Model_Instance"));
...
CComponent* CEditor_Model_Instance::Clone(void* pArg)
{
    CEditor_Model_Instance* pInstance = new CEditor_Model_Instance(*this);
...
void CEditor_Model_Instance::Free()
{
    __super::Free();
