...
class CLevel_Embars final : public CLevel
{
private:
	CLevel_Embars(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	virtual ~CLevel_Embars() = default;
...
#pragma region 맵 관련
    ...
    // 조명 + 조명에 달리는 이펙트 불러오기 ( 큰 불 )
    HRESULT Ready_BrazierLights(const _tchar* pDataFileName, LEVEL eCurrentLevel, KHAZAN_MAP eMap = KHAZAN_MAP::END);
