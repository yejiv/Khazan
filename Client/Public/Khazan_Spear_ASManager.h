...
public:
    ...
    {
        _uint               iFromCategory;
        kHAZAN_ANIM_INFO    toAnimInfo;
        function<_bool()>   checkCondition;
        _uint               iPriority;  //  특수 상황에서 이걸 사용하자

        tagKhazanSpearAnimationTransitionRule(_uint from, kHAZAN_ANIM_INFO to, function<_bool()> condition, _uint priority )
            : iFromCategory(from), toAnimInfo(to), checkCondition(condition), iPriority(priority) {
        }
...
public:
    ...
    // 전환 규칙 추가
    void Add_Transition(_uint iFrom, kHAZAN_ANIM_INFO toAnimInfo, function<_bool()> condition, _uint priority = 99);

    // 강제 상태 변경 (우선순위 무시)
    void Force_ChangeState(kHAZAN_ANIM_INFO animInfo);

    //플레이어 상태를 감지하다.
    kHAZAN_ANIM_INFO Detect_State(const vector<kHAZAN_ANIM_INFO>& candidates);

    // 업데이트
    void Update(_float fTimeDelta, kHAZAN_ANIM_INFO animInfo);
    ...
    _uint Get_CurrentCategory() const { return m_eCurState.animInfo.iCategory; }
    _int Get_CurrentAnimIndex() const { return m_eCurState.iAnimIndex; }


private:
    CKhazan_Spear_ASMachine*        m_pASM = { nullptr };
    ...
    //DIRECTION_INFO				m_ePlayerDir = {};

private:
    // 상태 변경
    void Change_State(kHAZAN_ANIM_INFO animInfo);

    // 전환 가능 여부 체크
    _bool Can_Transition(_uint iNewCategory, _uint iNewSubType);

    // 애니메이션 인덱스 찾기
    _int Find_AnimationIndex(kHAZAN_ANIM_INFO animInfo);

public:
    static CKhazan_Spear_ASManager* Create(CKhazan_Spear_ASMachine* pASM);
