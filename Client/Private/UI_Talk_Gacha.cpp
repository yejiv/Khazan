...
_bool CUI_Talk_Gacha::isEmptyNextEvent()
{
    return m_iNextEvent == 0 ? true : false;
}

_bool CUI_Talk_Gacha::isExistNextTalk()
{
...
_bool CUI_Talk_Gacha::isTalkingEnd()
{
    return m_isTalkingEnd;
}

_bool CUI_Talk_Gacha::isTalking()
{
    return m_eTaking == TALKSTATE::TALKING ? true : false;
}

HRESULT CUI_Talk_Gacha::On_Panel(_int iTalkIndex)
{
...
HRESULT CUI_Talk_Gacha::Initialize_Prototype()
{
    return S_OK;
}
...
void CUI_Talk_Gacha::Update_Font(_float fTimeDelta)
{
...
CUI_Talk_Gacha* CUI_Talk_Gacha::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
