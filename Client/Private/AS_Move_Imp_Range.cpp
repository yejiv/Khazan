...
void CAS_Move_Imp_Range::Enter(CStateMachine* pFSM, CGameObject* pOwner)
{
   ...
   /* CBlackBoard* BB = m_pGameInstance->Get_BlackBoard();*/
    ...
    m_fSpeed = pBB->Get_Value<_float>(pImp->Get_Name(),"WalkSpeed");
...
void CAS_Move_Imp_Range::Update(CStateMachine* pFSM, CGameObject* pOwner, _float fTimeDelta)
{
    ...
    _float fAttackRange = pBB->Get_Value<_float>(pImp->Get_Name(), "MagicRange");
    ...
    pImp->Get_Controller()->
        AI_MoveTo(pOwner,
            pBB->Get_Value<CGameObject*>(pImp->Get_Name(), "Target"),
            fDesireDist,
            fMoveSpeed,
            fTimeDelta);
