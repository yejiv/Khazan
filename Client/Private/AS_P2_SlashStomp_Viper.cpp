...
CAS_P2_SlashStomp_Viper::CAS_P2_SlashStomp_Viper()
{

}

void CAS_P2_SlashStomp_Viper::Enter(CStateMachine* pFSM, CGameObject* pOwner)
{
...
void CAS_P2_SlashStomp_Viper::Update(CStateMachine* pFSM, CGameObject* pOwner, _float fTimeDelta)
{
    ...
    if (pModel->Play_Animation(fTimeDelta))
    {
        ...
        pBB->Set_Value<_bool>(pViper->Get_Name(), "is_P2_SlashStompFinished", true);
    }
}
...
void CAS_P2_SlashStomp_Viper::OnCollision(COLLISION_DESC* pDesc, _uint iCollisionLayer, CGameObject* pOwner)
{
    ...
    if (COLLISION_LAYER::PLAYER == eLayer)
    {
        ...
        {
            pTarget->Take_Damage(92.f, HITREACTION::KNOCKBACK_WEAK);
            _vector vLook = pOwnerTransform->Get_State(STATE::LOOK);
            pTarget->KnockBack(vLook, 15.f, 60.f);
        }
...
{
    return new CAS_P2_SlashStomp_Viper();
}

void CAS_P2_SlashStomp_Viper::Free()
{
    __super::Free();
}
