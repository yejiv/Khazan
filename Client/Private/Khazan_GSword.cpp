...
HRESULT CKhazan_GSword::Initialize_Clone(void* pArg)
{
    ...
    /* ?îå?†à?ù¥?ñ¥ ?Öã?åÖ  */
    m_pPlayerData = m_pClientInstance->Get_pInitailizePlayerData();  //?îå?†à?ù¥?ñ¥ ?ç∞?ù¥?Ñ∞ ?ó∞Í≤?
...
#pragma region ?ÉÅ?ò∏ ?ûë?ö© Îß? ?ò§Î∏åÏ†ù?ä∏ ?ù¥Î≤§Ìä∏
    Subscribe_Events();
#pragma endregion
...
void CKhazan_GSword::Take_Damage(_float fDamage, HITREACTION eHitreaction, CGameObject* pGameObject)
{
    ...
    if (eHitreaction != HITREACTION::GRAB)
    {
        ...
        /* Í∞??ìú Ï§? Í∞ïÌïú?ÑâÎ∞±Í≥µÍ≤©Ïù¥ ?ò§Î©? ?Ñ±Í≥µÎ™®?Öò Ï∑®ÌïòÍ∏? */
    ...
    /* ?îå?†à?ù¥?ñ¥ Ï£ΩÏóà?ùÑ ?ïå ?Ñ∏?åÖ?ïò?äî Î≤?  */
...
void CKhazan_GSword::Update_Stats(_float fTimeDelta)
{
    ...
    if (m_pPlayerData->fCulStamina < 0.1f && !m_pAnimAttack->Is_Attacking() && !m_pAnimAttack->Is_Skilling())
    {
        ...
        if (!Has_Status(STAMINA_EXHAUSTION)) {
            ...
            m_pClientInstance->Set_PlayerInput(false);     //?ûÖ?†• ÎßâÍ∏∞ 
        }
        ...
        if (Has_Status(STAMINA_EXHAUSTION) && (m_pBody->Get_Model()->IsFinished()/* || m_pBody->Get_Model()->Get_CurAnimIndex() != m_pBody->Get_Model()->Get_AnimIndexByName("CA_P_Kazan_StaminaExhaustion")*/))
        {
            ...
            m_pPlayerData->fCulStamina += m_pPlayerData->fStaminaRegen;      //?ä§?ÉúÎØ∏ÎÇò ?ïΩÍ∞? ?öåÎ≥? 
            m_pClientInstance->Set_PlayerInput(true);       //?ûÖ?†• ???Í∏?
        }

        return;
    }

    //Î∞îÎîî?óê?Ñú Í∞??ìú ?Ç¨?ö© ?ãú Ï¥àÍ∏∞?ôî
    ...
    /* idle, run, walk, guard ?òÑ?û¨ ?ä§?ÉúÎØ∏ÎÇòÍ∞? ?ã≥?ïÑ ?ûà?äî ?ÉÅ?Éú?ùº ?ïå ?öåÎ≥µÌïòÍ∏? */
    if ((!Has_States()
...
void CKhazan_GSword::Update_State(_float fTimeDelta)
{
   ...
   /* Ladder Climb ?ÉÅ?Éú ÏµúÏö∞?Ñ† Ï≤¥ÌÅ¨ */
    ...
    // Í∞??ìú Ï§ëÏù¥Î©? ???ÎØ∏Ï?? Î¨¥Ïãú
    if (IsGuarding && IsDamaged)
    {
        // Í∞??ìú ?Ñ±Í≥? Ï≤òÎ¶¨?äî ?ó¨Í∏∞ÏÑú
        m_pAnimDamaged->Clear_Damaged();
        IsDamaged = false;
    }

    // Í∞??ìúÏ§ëÏù¥ ?ïÑ?ãàÍ≥?  ???ÎØ∏Ï?? ?ÉÅ?ÉúÍ∞? ?Åù?Ç¨?ùÑ ?ïåÎß? ?ÉÅ?Éú ?†úÍ±?
    ...
    // Í∞??ìúÏ§ëÏù¥ ?ïÑ?ãàÍ≥?  ???ÎØ∏Ï?? Î∞õÎäî Ï§ëÏù¥Î©? ?ûÖ?†• Î∞? ?ã§Î•? Î™®Îì† Ï≤òÎ¶¨ Ï∞®Îã®
    if (!IsGuarding && IsDamaged)
    {
        // ???ÎØ∏Ï?? Î°úÏßÅÎß? ?ã§?ñâ
        ...
        return;  // ???ÎØ∏Ï?? Ï§ëÏóê?äî ?ã§Î•? Î™®Îì† Ï≤òÎ¶¨ Ï∞®Îã®
    }
    ...
    if (m_pClientInstance->Get_PlayerInput())
    {
        ...
        /* Í≥µÍ≤©?ù¥?Çò Í∞??ìúÎ•? ÎßâÏïÑ?ïº?ïò?äî ?ÉÅ?ò∏?ûë?ö© Ï≤òÎ¶¨  */
        ...
        // Í≥µÍ≤© Ï§ëÏùº ?ïå?äî Move_Input?ùÑ ?ôÑ?†Ñ?ûà Ï∞®Îã®
...
_bool CKhazan_GSword::Fall_Input(_float fTimeDelta)
{
    if (Has_Status(FALLING | PRE_LAND | FALLING_ATTACK))
    {
        ...
        {
            // ?Çô?ïòÍ≥µÍ≤© ?ûÖ?†• Ï≤¥ÌÅ¨ (?Çô?ïò Ï§ëÏóêÎß?)
        ...
        // ?Çô?ïò/Ï∞©Ï?? ?ï†?ãàÎ©îÏù¥?Öò Í≥ÑÏÜç ?ã§?ñâ
        m_pAnimFall->Continue(fTimeDelta);

        // Ï∞©Ï?? ?ï†?ãàÎ©îÏù¥?Öò ?ôÑÎ£? Ï≤¥ÌÅ¨
...
{
    /*  Í≥µÍ≤©Ï§? ?ã∑Ïß??äî ?òà?ô∏ Ï≤òÎ¶¨ (Í≥µÍ≤©Ï§? Îπ†Î•∏ ????ù¥Î∞çÏóê ?ã∑Ïß??ï† ?àò ?ûà?èÑÎ°?) */
    ...
    // Í≥µÍ≤©/Í∞??ìú Ï§ëÏùº ?ïå?äî ?ôÑ?†Ñ?ûà Î¶¨ÌÑ¥
    if (m_pAnimAttack->Is_Attacking() || m_pAnimGuard->Is_Guarding())
    {
        // Í≥µÍ≤© Ï§ëÏùº ?ïå?äî Move ?ÉÅ?Éú?èÑ ?†úÍ±?
        if (Has_State(CAT::M_MOVE))
        {
    ...
    if (isPrevDodge)
    {
        ...
        //// ?ã∑Ïß? ?ÉÅ?Éú ?ú†Ïß? - ?ï†?ãàÎ©îÏù¥?Öò?ù¥ ÏßÑÌñâ Ï§ëÏù¥Î©? DODGING ?ÉÅ?Éú Ï∂îÍ??
        //if (!m_pAnimMove->IsStopMoveAnimantionFinished())
        //{
        //    Add_Status(DODGING);  // ?ã∑Ïß? Ï§? ?ÉÅ?Éú ?îå?ûòÍ∑? ?Ñ§?†ï
        //}

        // Dodge ?ï†?ãàÎ©îÏù¥?Öò?ù¥ ?Åù?Ç¨?äîÏß? ?ôï?ù∏
        if (m_pAnimMove->IsStopMoveAnimantionFinished()&& !m_pAnimMove->IsDodge()&& !Has_Status(DODGE_ENDING))
        {
            //Remove_SubState(MOV::MOVE_DODGE);
           // Remove_Status(CHARGING_SPRINT);
            // Î∞©Ìñ•?Ç§Îß? ?àå?†§?ûà?úºÎ©? Ï¶âÏãú Run?úºÎ°? ?†Ñ?ôò
            ...
            return;  //Dodge Ï¢ÖÎ£å Ï≤òÎ¶¨ ?õÑ Î∞îÎ°ú Î¶¨ÌÑ¥
        }
        else
        {
            //Dodge ?ï†?ãàÎ©îÏù¥?Öò ?û¨?Éù Ï§ëÏù¥Î©? ?ûÖ?†• Î¨¥Ïãú, ?ÉÅ?Éú?äî ?ú†Ïß?
            return;
        }
    }
    ...
    if (Has_State(CAT::M_MOVE))
    {
        ...
        /* ?ä§?éò?ù¥?ä§ ?ñºÍ≥? Î∞©Ìñ•?Ç§Î•? ?àÑÎ•¥Í≥† ?ûà?ã§?äî ?òà?ïΩ?ù¥ Í±∏Î†§?ûà?ùÑ ?ïå ?ã§?ãú ?ä§?éò?ù¥?ä§Î•? ?àÑÎ•? Í≤ΩÏö∞*/
        ...
        else if (m_pGameInstance->Key_Pressing(DIK_SPACE, fTimeDelta, INPUT_TYPE::GAMEPLAY, &m_fSprintTime))
        {
            if (Has_Status(CHARGING_SPRINT))
            {
                ...
                // Dodge ?ï†?ãàÎ©îÏù¥?Öò ÏßÑÌñâ?èÑ Ï≤¥ÌÅ¨
                ...
                {
                    Add_SubState(MOV::MOVE_SPRINT);
                    Remove_Status(CHARGING_SPRINT);  // Sprint ?†Ñ?ôò ?ôÑÎ£?
                }
                else
                {
                    Add_SubState(MOV::MOVE_DODGE);  // ?ïÑÏß? Dodge ?ú†Ïß?
                    isSpaceHandled = true;
                }
            }
            else
            {
                // ?ù¥ÎØ? Sprint ?ÉÅ?Éú - ?ú†Ïß?
                Add_SubState(MOV::MOVE_SPRINT);
                isSpaceHandled = true;
            }
        }
        ...
        else if (m_pGameInstance->Key_Up(DIK_SPACE))
        {
            ...
            // Sprint Ï¢ÖÎ£å ?ã†?ò∏ (?ã§?ùå ?îÑ?†à?ûÑ?óê END ?ï†?ãàÎ©îÏù¥?Öò ?û¨?Éù)
    ...
    else
    {
        ...
        if (m_pGameInstance->Key_Down(DIK_SPACE))
        {
            ...
            Add_CycleState(CYC::CYCLE_END);
            m_eDir.Clear_Flag();
            m_eDir.Add_Flag(DIR::B);  // ?í§Î°? ?öå?îº
            m_fSprintTime = 0.f;
            Add_Status(BACK_DODGE );
        ...
        {
            // ?ù¥?èô?ïòÏß? ?ïä?ùÑ ?ïå?äî SubState Ï¥àÍ∏∞?ôî
            Clear_SubState();
            m_fSprintTime = 0.f;
            Remove_Status(CHARGING_SPRINT);
        }
...
{
    // ?ä§?Ç¨?ù¥ ?Åù?Ç¨?äîÏß? Ï≤¥ÌÅ¨
    if (Has_State(CAT::M_SKILL) && m_pAnimAttack && !m_pAnimAttack->Is_Skilling())
    {
       // m_pClientInstance->Set_UsedSkill(m_iCurSkillIndex, false); //?Ç¥Î∂??óê?Ñú ?ï¥Í≤? 
        Clear_Step1();
        return false;
    }

    ///* ?à¨Ïß?, ?ä§?ÉúÎØ∏ÎÇò  ?ä§?ÉØ ?ôï?ù∏ - attack ?ïà?óê?Ñú Í≤??Ç¨ */
...
_bool CKhazan_GSword::Attack_Input(_float fTimeDelta)
{
    ...
    /* Í∞ïÌïú Í≥µÍ≤© 1???  + Ï∞®Ïßï + ?ä§?Ç¨: ?à®?Üµ?ÅäÍ∏∞Îì§ ?åêÎ≥? */
    ...
    else if (m_pGameInstance->Mouse_Pressing(MOUSEKEYSTATE::RB))
    {
        if (!Has_Status(CHARGING_STRONG_ATTACK))
        {
            ...
            if (m_fChargingStrongAttackTime >= m_fChargingStartIntervalTime)
            {
                ...
                /* ?òà?ïΩÏ§ëÏù∏ Í≥µÍ≤© ???Í∏∞Í?? ?ûà?úºÎ©? true */
   ...
   // if ((m_iPrevMainState & CAT::M_MOVE) && (m_iPrevSubState & MOV::MOVE_DODGE) && (m_pGameInstance->Mouse_Down(MOUSEKEYSTATE::LB) || m_pGameInstance->Mouse_Down(MOUSEKEYSTATE::RB)))
    ...
    /* Sprint Í≥µÍ≤©  - ?ï†?ãàÎ©îÏù¥?Öò ?ûêÏ≤¥Í?? ?ù¥?ÉÅ?ï®. ?ïú ?îÑ?†à?ûÑ ?àúÍ∞ÑÏóê ?àúÍ∞ÑÏù¥?èô?ï® - ?Ç¨?ö© xxx */
...
_bool CKhazan_GSword::Guard_Input(_float fTimeDelta)
{
    ...
    //   /* Í∞??ìú ?Ñ±Í≥?  */
       //else if (Has_State(CAT::M_GUARD)&&) //todo Ï°∞Í±¥ Ï£ºÍ∏∞
       //{
       ...
       /* Í∞??ìúÏ§? ?ù¥?èô - Î∞©Ìñ• ?ûÖ?†•?ù¥ ?ûà?ùÑ ?ïå */
    if (Has_State(CAT::M_GUARD) && m_eDir.iDirFlag > 0)
    {
        // Î∞©Ìñ•?ù¥ Î∞îÎ?åÏóàÍ±∞ÎÇò, ?õå?ÇπÍ∞??ìúÍ∞? ?ïÑ?ãê ?ïå
...
{
    /* ?Ç¨?ã§Î¶? ???Í∏? ?Åù?ÇòÍ≥? Î¨¥Í∏∞ Í∫ºÎÇº?ïå ÍπåÏ?? */
...
{
    // ?Çô?ïò Ï§ëÏóê?äî ?ï†?ãàÎ©îÏù¥?Öò Î≥?Í≤? Í∏àÏ??
    ...
    /* ?ùΩ?ò®?ÉÅ?Éú?óê?Ñú ?ã∑Ïß?Ï§ëÏùº ?ïå?äî Î∞©Ìñ• Î≥?Í≤? Í±¥ÎÑà?öú?Ö£Í∏?*/
...
void CKhazan_GSword::Apply_PlayerMovement(_float fTimeDelta)
{
    ...
    // ?Çô?ïò Ï§ëÏóê?äî ?ù¥?èô Í∏àÏ??
    if (Has_Status(FALLING | FALLING_ATTACK | PRE_LAND))
        return;

    // Í≥µÍ≤© Ï§ëÏùº ?ïå?äî ?ù¥?èô?ïòÏß? ?ïä?ùå
    if (m_pAnimAttack->Is_Attacking()) {
        //?ä§?îÑÎ¶∞Ìä∏ Ï≤¥ÌÅ¨ Í∞ïÌôî
        if (Has_SubState(ATT::ATK_SPRINTATK))
            return;

        //?ï†?ãàÎ©îÏù¥?Öò ÏµúÏÜåÎ≥¥Ïû•?ãúÍ∞?
        if (!m_pBody->Get_Model()->Check_MinAnimationTime())
            return;
    }
    /* ?ã∑Ïß? ?ùº?ïå ?ù¥?èô?ïòÏß? ?ïä?ùå*/
    if (Has_State(CAT::M_MOVE) && Has_SubState(MOV::MOVE_DODGE))
        return;

    /* stop ?ï†?ãàÎ©îÏù¥?Öò?èÑ ?ù¥?èô x  */
    ...
    // ?í§ Î∞©Ìñ• ?ã∑Ïß??ù∏Ïß? Ï≤¥ÌÅ¨ (B, BR, BL Î™®Îëê ?è¨?ï®)
    ...
    if (!isBackwardDodge) {
        ...
        else
        {
            ...
            if (Has_State(CAT::M_MOVE) && isMoving)
            {
                ...
                {
                    // Í∞ÅÎèÑ Ï∞®Ïù¥Í∞? ?ûë?ùÑ ?ïå Ï¶âÏãú ?öå?†Ñ
                    _float yaw = atan2f(XMVectorGetX(vTargetLook), XMVectorGetZ(vTargetLook));
                    _vector q = XMQuaternionRotationAxis(XMVectorSet(0.f, 1.f, 0.f, 0.f), yaw);
                    m_pTransformCom->Set_Quaternion(q);
                }
...
void CKhazan_GSword::Check_KeyInput_Direction(_float fTimeDelta)
{
    ...
    // Ïπ¥Î©î?ùº Í∏∞Ï?? ?õî?ìú Î∞©Ìñ• (?ù¥?èô/?öå?†Ñ?ö©)
    ...
    // ?îå?†à?ù¥?ñ¥ Î°úÏª¨ Î∞©Ìñ• (?ï†?ãàÎ©îÏù¥?Öò ?Ñ†?Éù?ö©)
    m_eDir.Clear_Flag();
...
DIRECTION_INFO CKhazan_GSword::Calculate_LockOnDirection(_float fTimeDelta)
{
    ...
    // ?ù¥?èô Î∞©Ìñ•?ùÑ ???Í≤? Í∏∞Ï?? Ï¢åÌëúÍ≥ÑÎ°ú Î≥??ôò
    // Forward = ???Í≤? Î∞©Ìñ•, Right = ???Í≤üÏùò ?ò§Î•∏Ï™Ω
...
{
    // ?ùΩ?ò® ?ÉÅ?Éú?óê?Ñú?äî ?ï≠?ÉÅ ???Í≤üÏùÑ Î∞îÎùºÎ≥¥Í≤å
    _float4* pLockOnPos = m_pCamera->Get_LockOnPosition();
    if (pLockOnPos)
    {
        _vector vTargetPos = XMLoadFloat4(pLockOnPos);
...
{
    // Ïπ¥Î©î?ùº?ùò ?ùΩ?ò® ?ÉÅ?Éú??? ?èôÍ∏∞Ìôî
    if (m_pCamera)
    {
...
void CKhazan_GSword::Get_HitReaction(_float3 vContactPoint)
{
    ...
    if (fLengthSq < 0.0001f) // Í±∞Ïùò 0?ù∏ Í≤ΩÏö∞
    {
        // Í∏∞Î≥∏Í∞?: Ï∫êÎ¶≠?Ñ∞?ùò ?†ïÎ©? Î∞©Ìñ•?ùÑ ?Ç¨?ö©
    ...
    // 5. Ï∫êÎ¶≠?Ñ∞?ùò Forward??? Right Î≤°ÌÑ∞ Íµ¨ÌïòÍ∏?
...
void CKhazan_GSword::Check_IsInAir(_float fTimeDelta)
{
    ...
    ))
    {
        ...
        if (fFraction <= 0.2f)
        {
            // ?Çô?ïò Ï§ëÏù¥?óà?ã§Î©? Ï∞©Ï?? ?ôÑÎ£?
            if (Has_Status(FALLING | FALLING_ATTACK | PRE_LAND))
            {
                // PRE_LAND ?ÉÅ?ÉúÍ∞? ?ïÑ?ãàÎ©? Î∞îÎ°ú PRE_LANDÎ°? ?†Ñ?ôò
        ...
        /* Ï∞©Ï?? ÏßÅÏ†Ñ - Ï∞©Ï?? ?ï†?ãàÎ©îÏù¥?Öò Ï§?Îπ? */
...
{
    // 8Î∞©Ìñ• Î≥??ôò ?Öå?ù¥Î∏?
    // [?îå?†à?ù¥?ñ¥Í∞? Ïπ¥Î©î?ùº Í∏∞Ï???úºÎ°? Î≥¥Îäî Î∞©Ìñ•][?ûÖ?†• Î∞©Ìñ•] = ?îå?†à?ù¥?ñ¥ Í∏∞Ï?? Î∞©Ìñ•
    static const _uint conversionTable[8][8] = {
        // Ïπ¥Î©î?ùº ?ûÖ?†•:    F,  R,  B,  L,  FR, BR, BL, FL
...
{
    // ?ÉÅ?ò∏ ?ûë?ö© ?ò§Î∏åÏ†ù?ä∏ Ï™ΩÏóê?Ñú BEGIN STATE ?Ç¥Î≥¥ÎÇ¥Î©? ?îå?†à?ù¥?ñ¥?óê?Ñú ?ñâ?èô ?õÑ, ?ñâ?èô ?ôÑÎ£? ?ãú ?ù¥Î≤§Ìä∏ Î∞úÏÉù?úºÎ°? ?ÉÅ?ò∏ ?ûë?ö© ?ò§Î∏åÏ†ù?ä∏ ?èô?ûë
    if (EventInteractType::EVENT_STATE::BEGIN == m_EventInteract.eState)
    {
        if (false == m_isInteractEventSetting)
        {
            ...
            /*  UnArmed ?ï†?ãàÎ©îÏù¥?Öò ?û¨?Éù  (Ï°∞Í∞Å?ÉÅ?ïå?äî ?ïà?ï®)*/
        ...
        // ?îå?†à?ù¥?ñ¥ ?ù¥?èô, LOOK Î≥¥Í∞Ñ?? | ?ôÑÎ£åÌïòÎ©? ?ù¥Î≤§Ìä∏ Î∞òÎ??Î°? ?çò?†∏Ï£ºÍ∏∞
        _bool isDone = { true };
        ...
        {
            isDone = false;
            if (m_pBody->Get_Model()->IsFinished())  isDone = true;   /* ?òÑ?û¨ ?û¨?Éù?êò?äî ?ï†?ãàÎ©îÏù¥?Öò?ù¥ UnArmed?ù¥Í≥? ?Åù?Ç¨?úºÎ©? trueÎ°? */
            break;
        }
        ...
        /* ?îå?†à?ù¥?ñ¥Í∞? ?ò§Î∏åÏ†ù?ä∏?ïú?Öå "?Çò ?ù¥?†ú Ï§?ÎπÑÎêê?ñ¥"Î•? Î≥¥ÎÇ∏?ã§. */
        if (isDone)               // ?äπ?†ï Ï°∞Í±¥ ?ôÑ?Ñ±?ïòÎ©? ?ù¥Î≤§Ìä∏ Î∞úÏÉù
        {
            // ?ù¥Î≤§Ìä∏?óê ?ïÑ?öî?ïú ?Ñ∏?åÖ?ùÑ ?ã§?ùå?óê ?òê Î∞úÏÉù?ãú Î≥?Í≤? Í∞??ä•?ïòÍ≤? falseÎ°? Î≥?Í≤?
            m_isInteractEventSetting = false;

            // ?ÉÅ?ò∏?ûë?ö© ?ôú?Ñ±?ôî?ãú Îß? ?ò§Î∏åÏ†ù?ä∏?ïú?Öå EVENT_STATEÎ•? ON ?úºÎ°? ?çò?†∏Ï§??ã§
            _float4 vPosition = {};
            ...
            // ?Ç¥ ?ÉÅ?ÉúÎ•? STATE::NONE ?úºÎ°? Î≥?Í≤ΩÌï¥Ï§??ã§.
            m_EventInteract.eState = EventInteractType::EVENT_STATE::NONE;
        }
    }

    // ?ÉÅ?ò∏ ?ûë?ö© ?ò§Î∏åÏ†ù?ä∏ Ï™ΩÏóê?Ñú END STATE ?Ç¥Î≥¥ÎÇº ?ãú
    if (EventInteractType::EVENT_STATE::END == m_EventInteract.eState)
    {
        if (true)               // ?äπ?†ï Ï°∞Í±¥ ?ôÑ?Ñ±?ïòÎ©? ?ù¥Î≤§Ìä∏ Î∞úÏÉù
        {
            // ?ÉÅ?ò∏?ûë?ö© ÎπÑÌôú?Ñ±?ôî?ãú Îß? ?ò§Î∏åÏ†ù?ä∏?ïú?Öå EVENT_STATEÎ•? OFF Î°? ?çò?†∏Ï§??ã§
...
void CKhazan_GSword::BladeNexus_Event(_float fTimeDelta)
{
    ...
    // Í∑?Í≤??óê ?†ëÏ¥? ?õÑ ?ÉÅ?ò∏ ?ûë?ö© ( Í∑?Í≤? Í∞??èô )
    if (false == BNEvent.isBNOpened)
    {
        ...
        {
            // Ï≤? ?ï¥Í∏? ?îå?†à?ù¥?ñ¥    ?ï†?ãàÎ©îÏù¥?Öò ?û¨?Éù 
        ...
        {
            // ?ï¥Í∏àÎêú Í∑?Í≤? ?îå?†à?ù¥?ñ¥ ?ï†?ãàÎ©îÏù¥?Öò ?û¨?Éù
        ...
        // ?îå?†à?ù¥?ñ¥ Look -> Í∑?Í≤? ( Í∏∞Ïö∞?äîÍ±? Î≥¥Ï†ï?ïò?†§Í≥? ?ù¥?†áÍ≤? ÏΩîÎìú ?Ñ£?ñ¥?Ü®?äµ?ãà?ã§. )
    ...
    // Í∑?Í≤? Í∞??èô ?Åù?ÇòÍ≥? UI ?åù?óÖ ( Í∑?Í≤? UI Ï∞? ?ôú?Ñ±?ôî ) ( ?îå?†à?ù¥?ñ¥?äî LOOP ?ï†?ãàÎ©îÏù¥?Öò )
    else if (true == BNEvent.isBNOpened)
    {
        // Í∑?Í≤? Ï≤? ?ï¥Í∏? ?ãú
        if (true == BNEvent.isUnLock)
        {
            // Ï≤? ?ï¥Í∏? ?îå?†à?ù¥?ñ¥ Í∑?Í≤? LOOP Animation?
        }
        // ?ù¥ÎØ? ?ï¥Í∏àÎêú Í∑?Í≤?
        else if (false == BNEvent.isUnLock)
        {
            // ?ï¥Í∏? ?êú ?îå?†à?ù¥?ñ¥ Í∑?Í≤? LOOP Animation?
        }
    }
...
void CKhazan_GSword::Chest_Event(_float fTimeDelta)
{
    ...
    // ?ÉÅ?ûê?óê ?†ëÏ¥? ?õÑ ?ÉÅ?ò∏ ?ûë?ö© ( ?ã´?ûå ?ÉÅ?Éú )
    if (false == ChestEvent.isChestOpened)
    {
        ...
        // ?îå?†à?ù¥?ñ¥ Look -> ?ÉÅ?ûê, Position ?ÉÅ?ûê Î≥? ?úÑÏπòÎ°ú ?ù¥?èô ( Í∏∞Ïö∞?äîÍ±? Î≥¥Ï†ï )
    ...
    // ?ÉÅ?ûê ?ó¥Î¶¨Îäî ?ï†?ãàÎ©îÏù¥?Öò Ï¢ÖÎ£å?êòÎ©? ( ?ó¥Î¶? ?ÉÅ?Éú )
...
void CKhazan_GSword::TombStone_Event(_float fTimeDelta)
{
    ...
    // ?àº?ä§?Ü§ Í∞??èô ?Åù?ÇòÍ≥? Í∞??èô LOOP ÏßÑÏûÖ
...
void CKhazan_GSword::Lever_Event(_float fTimeDelta)
{
    ...
    if (EventLever::ACTIVE == LeverEvent.eState)
    {
        ...
        // ?îå?†à?ù¥?ñ¥ Look -> ?†àÎ≤?, Position ?†àÎ≤? Î≥? ?úÑÏπòÎ°ú ?ù¥?èô ( Í∏∞Ïö∞?äîÍ±? Î≥¥Ï†ï )
    ...
    // ?†àÎ≤ÑÍ?? DeActive Î°? ?†Ñ?ôò Ï§ëÏùº ?ïå
    else if (EventLever::DEACTIVE == LeverEvent.eState)
    {
        // ?îå?†à?ù¥?ñ¥Í∞? ?†àÎ≤ÑÎ?? DeActive ?ãú?Ç§?äî ?ï†?ãàÎ©îÏù¥?Öò ?û¨?Éù
...
void CKhazan_GSword::Ladder_Event(_float fTimeDelta)
{
    ...
    default:
        MSG_BOX(TEXT("?ù≠ ?Ç¨?ã§Î¶? ?ù¥?ü∞Í±? ?óÜ?äî?îî"));
        break;
    }

    if (true == LadderEvent.isStartAction())
    {
        LadderEvent.vPlayerPosition.y = m_pTransformCom->Get_State(STATE::POSITION).m128_f32[1] + 0.1f ; //?îå?†à?ù¥?ñ¥Í∞? ?ù¥?èô?ï† ?úÑÏπ? y 

        // ?îå?†à?ù¥?ñ¥ Look -> ?†àÎ≤?, Position ?†àÎ≤? Î≥? ?úÑÏπòÎ°ú ?ù¥?èô ( Í∏∞Ïö∞?äîÍ±? Î≥¥Ï†ï )
        m_pTransformCom->Set_State(STATE::POSITION, XMLoadFloat4(&LadderEvent.vPlayerPosition));  //?àúÍ∞ÑÏù¥?èô
        LadderEvent.vPosition.y = m_pTransformCom->Get_State(STATE::POSITION).m128_f32[1]; // ?†à?çî ?úÑÏπ? 
        m_pTransformCom->LookAt(XMLoadFloat4(&LadderEvent.vPosition)); //Íº¨ÎùºÎ≥¥Í∏∞ 
    }
...
void CKhazan_GSword::Debug_Widget_Guard()
{
    ...
    // ?ó¨Í∏∞ÏÑú Body?ùò ?†ïÎ≥¥Î?? Í∞??†∏?ò¨ ?àò ?ûà?ã§Î©? ?ëú?ãú
...
//
//    // ?Éù?Ñ†?êú commandsÎ°? ?ñ¥?ñ§ ?ï†?ãàÎ©îÏù¥?Öò?ùÑ ?û¨?Éù?ï†Ïß? ?†ï?ïòÍ∏? 
//    Update_State(fTimeDelta);
//
//    // ?ï†?ãàÎ©îÏù¥?Öò ?ã§?ñâ Î∞? ?ÉÅ?ÉúÎ≥? Î°úÏßÅ ?ã§?ñâ?ãú?Ç§Í∏? (ÏµúÏÜå Î≥¥Ïû• ?ãúÍ∞ÑÏúºÎ°? ?úÑ?óê?Ñú ?†ï?ïú ?ï†?ãàÎ©îÏù¥?Öò Î¨¥Ïãú?ïòÍ∏? )
//    Update_Animation(fTimeDelta);
//
//    // ?ù¥?èô/?öå?†Ñ ?†Å?ö©
//    Update_Movement(fTimeDelta);
//
//    // ?†Ñ?à¨ Î°úÏßÅ( hp / stamina / status Í¥?Î¶? / ?ÑâÎ∞?..?  )
