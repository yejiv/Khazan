...
CJar_7th::CJar_7th(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
    : CDanjinJar{ pDevice, pContext }
{
}

CJar_7th::CJar_7th(const CJar_7th& Prototype)
    : CDanjinJar{ Prototype }
{
}

HRESULT CJar_7th::Initialize_Prototype()
{
...
HRESULT CJar_7th::Initialize_Clone(void* pArg)
{
...
void CJar_7th::Priority_Update(_float fTimeDelta)
{
    Find_Target();
...
void CJar_7th::Update(_float fTimeDelta)
{
    ...
    m_pGameInstance->Set_LightPosition(TEXT("DanjinJar_7"), ENUM_CLASS(LEVEL::EMBARS), vPosition);
}

void CJar_7th::Late_Update(_float fTimeDelta)
{
...
HRESULT CJar_7th::Render()
{
    CHECK_FAILED_MSG(Bind_ShaderResources(), TEXT("CJar_7th : Bind_ShaderResources 함수 E_FAIL"), E_FAIL);
...
HRESULT CJar_7th::Ready_Components(void* pArg)
{
...
HRESULT CJar_7th::Ready_TalkUI(void* pArg)
{
    ...
    Desc.szName = "DanjinJar_7th_TalkUI";
...
HRESULT CJar_7th::Bind_Materials(_uint iMeshIndex)
{
    m_iMtrlFlags = 0;
...
void CJar_7th::Animation_Update(_float fTimeDelta)
{
...
void CJar_7th::Animation_Change(_float fTimeDelta)
{
    switch (m_eAnimState)
    {
...
void CJar_7th::Update_Step(_float fTimeDelta)
{
...
_float4 CJar_7th::Get_NextStepPos()
{
...
void CJar_7th::Collision_Enter(COLLISION_DESC* pDesc, _uint iOtherObjectLayer, _float3 vContactPoint, _float3 ContactNormal, COLLISION_DESC* pMyDesc)
{
...
void CJar_7th::Collision_Stay(COLLISION_DESC* pDesc, _uint iOtherObjectLayer, _float3 vContactPoint, _float3 ContactNormal, COLLISION_DESC* pMyDesc)
{
    if (iOtherObjectLayer == ENUM_CLASS(COLLISION_LAYER::CAMERA) || iOtherObjectLayer == ENUM_CLASS(COLLISION_LAYER::MONSTER))
        return;
...
void CJar_7th::Collision_Exit(COLLISION_DESC* pDesc, _uint iOtherObjectLayer, COLLISION_DESC* pMyDesc)
{
    if (iOtherObjectLayer == ENUM_CLASS(COLLISION_LAYER::CAMERA) || iOtherObjectLayer == ENUM_CLASS(COLLISION_LAYER::MONSTER))
        return;
...
CJar_7th* CJar_7th::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
    CJar_7th* pInstance = new CJar_7th(pDevice, pContext);

    if (FAILED(pInstance->Initialize_Prototype()))
    {
        MSG_BOX(TEXT("Failed To Created : CJar_7th"));
...
CGameObject* CJar_7th::Clone(void* pArg)
{
    CJar_7th* pInstance = new CJar_7th(*this);

    if (FAILED(pInstance->Initialize_Clone(pArg)))
    {
        MSG_BOX(TEXT("Failed To Cloned : CJar_7th"));
        Safe_Release(pInstance);
    }
