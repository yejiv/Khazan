...
CJar_6th::CJar_6th(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
    : CDanjinJar { pDevice, pContext }
{
}

CJar_6th::CJar_6th(const CJar_6th& Prototype)
    : CDanjinJar { Prototype }
{
}

HRESULT CJar_6th::Initialize_Prototype()
{
...
HRESULT CJar_6th::Initialize_Clone(void* pArg)
{
...
{
    CHECK_FAILED_MSG(Bind_ShaderResources(), TEXT("CJar_6th : Bind_ShaderResources 함수 E_FAIL"), E_FAIL);
...
HRESULT CJar_6th::Ready_Components(void* pArg)
{
...
HRESULT CJar_6th::Ready_TalkUI(void* pArg)
{
    ...
    Desc.szName = "DanjinJar_6th_TalkUI";
...
HRESULT CJar_6th::Bind_Materials(_uint iMeshIndex)
{
    m_iMtrlFlags = 0;
...
void CJar_6th::Animation_Update(_float fTimeDelta)
{
...
void CJar_6th::Animation_Change(_float fTimeDelta)
{
    switch (m_eAnimState)
    {
...
void CJar_6th::Update_Step(_float fTimeDelta)
{
...
_float4 CJar_6th::Get_NextStepPos()
{
...
void CJar_6th::Collision_Enter(COLLISION_DESC* pDesc, _uint iOtherObjectLayer, _float3 vContactPoint, _float3 ContactNormal, COLLISION_DESC* pMyDesc)
{
...
void CJar_6th::Collision_Stay(COLLISION_DESC* pDesc, _uint iOtherObjectLayer, _float3 vContactPoint, _float3 ContactNormal, COLLISION_DESC* pMyDesc)
{
    if (iOtherObjectLayer == ENUM_CLASS(COLLISION_LAYER::CAMERA) || iOtherObjectLayer == ENUM_CLASS(COLLISION_LAYER::MONSTER))
        return;
...
void CJar_6th::Collision_Exit(COLLISION_DESC* pDesc, _uint iOtherObjectLayer, COLLISION_DESC* pMyDesc)
{
    if (iOtherObjectLayer == ENUM_CLASS(COLLISION_LAYER::CAMERA) || iOtherObjectLayer == ENUM_CLASS(COLLISION_LAYER::MONSTER))
        return;
...
CJar_6th* CJar_6th::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
    CJar_6th* pInstance = new CJar_6th(pDevice, pContext);

    if (FAILED(pInstance->Initialize_Prototype()))
    {
        MSG_BOX(TEXT("Failed To Created : CJar_6th"));
...
CGameObject* CJar_6th::Clone(void* pArg)
{
    CJar_6th* pInstance = new CJar_6th(*this);

    if (FAILED(pInstance->Initialize_Clone(pArg)))
    {
        MSG_BOX(TEXT("Failed To Cloned : CJar_6th"));
        Safe_Release(pInstance);
    }
