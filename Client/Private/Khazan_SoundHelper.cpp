...
HRESULT CKhazan_SoundHelper::Initialize()
{
    ...
    EmplaceGroup(MOVE_SPRINT_START, TEXT("vo_kz_effort_sprint_01 (Korean(KR))"), 15);
    EmplaceGroup(MOVE_SPRINT, TEXT("footstep_kz_run_ice_01 (SFX)"), 23);
	...
	EmplaceGroup(ATTACK_SPEAR_STRONG3_FOLEY, TEXT("efx_spear_strongatk02_foley_01 (SFX)"), 4);
	...
	EmplaceGroup(ATTACK_SPEAR_WEAK_SPRINT, TEXT("efx_spear_trance_swish_01 (SFX)"), 7);
	EmplaceGroup(ATTACK_SPEAR_STRONG_SPRINT, TEXT("PLYR_Spear_Hit_Gore_Strong_01 (SFX)"), 3);
    EmplaceGroup(ATTACK_SPEAR_BRUTAL1_START, TEXT("efx_spear_frenzy01_foley_01 (SFX)"), 3);
	...
	// 나선 찌르기
	EmplaceGroup(SKILL_SPEAR_SPIRAL_THRUST_START, TEXT("efx_spear_spiralspear_start_01 (SFX)"), 1);
    ...
    //포즈
    EmplaceGroup(SPEAR_POSE_RETURN, TEXT("efx_spear_pose_return_01 (SFX)"), 4);
...
const TCHAR* CKhazan_SoundHelper::Get_NextSoundKey(PC_SOUND_GROUPTYPE type, PC_SOUND_CHANNEL eChannel)
{
...
void CKhazan_SoundHelper::MakeSoundKeyHelper(_wstring baseKey, size_t count, vector<_wstring>& resultKeys)
{
    ...
    size_t digit_start = _wstring::npos;
    size_t digit_end = _wstring::npos;
    size_t prefix_end = _wstring::npos;
    size_t initial_number = 0;

    for (size_t i = baseKey.size(); i > 0; --i)
    {
        _tchar current_char = baseKey[i - 1];

        if (iswdigit(current_char))
        {
            if (digit_end == _wstring::npos)
                digit_end = i;
        }
        else if (current_char == L'_' && digit_end != _wstring::npos)
        {
            digit_start = i;
            prefix_end = i - 1;
            break;
        ...
        {
            digit_end = _wstring::npos;
        }
    }

    // 패턴(_[숫자])을 찾지 못했거나 유효하지 않은 형식인 경우 그냥 baseKey 주기 
    if (digit_start == _wstring::npos || digit_end == _wstring::npos)
    {
        resultKeys.emplace_back(baseKey);
        return;
    }

    _wstring number_str = baseKey.substr(digit_start, digit_end - digit_start);
    _int initial_val = _wtoi(number_str.c_str());

    _wstring prefix = baseKey.substr(0, prefix_end + 1); 
    _wstring suffix = baseKey.substr(digit_end);  
    ...
    for (size_t i = 0; i < count; ++i)
    {
        ...
        int current_number = initial_val + (int)i;

        std::wstringstream wss;
        wss << prefix << std::setfill(L'0') << std::setw(2) << current_number << suffix;
 
        resultKeys.emplace_back(wss.str());
    }
