...
CJar_4th::CJar_4th(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
    : CDanjinJar{ pDevice, pContext }
{
}

CJar_4th::CJar_4th(const CJar_4th& Prototype)
    : CDanjinJar{ Prototype }
{
}

HRESULT CJar_4th::Initialize_Prototype()
{
...
HRESULT CJar_4th::Initialize_Clone(void* pArg)
{
...
void CJar_4th::Priority_Update(_float fTimeDelta)
{
    Find_Target();
}

void CJar_4th::Update(_float fTimeDelta)
{
    ...
    m_pGameInstance->Set_LightPosition(TEXT("DanjinJar_4"), ENUM_CLASS(LEVEL::EMBARS), vPosition);
}

void CJar_4th::Late_Update(_float fTimeDelta)
{
    m_pGameInstance->Add_RenderGroup(RENDERGROUP::DYNAMIC, this);
}

HRESULT CJar_4th::Render()
{
    CHECK_FAILED_MSG(Bind_ShaderResources(), TEXT("CJar_4th : Bind_ShaderResources 함수 E_FAIL"), E_FAIL);
...
HRESULT CJar_4th::Ready_Components(void* pArg)
{
...
HRESULT CJar_4th::Ready_TalkUI(void* pArg)
{
    ...
    Desc.szName = "DanjinJar_4th_TalkUI";
...
HRESULT CJar_4th::Bind_Materials(_uint iMeshIndex)
{
    m_iMtrlFlags = 0;
...
void CJar_4th::Animation_Update(_float fTimeDelta)
{
...
void CJar_4th::Animation_Change(_float fTimeDelta)
{
    switch (m_eAnimState)
    {
...
void CJar_4th::Update_Step(_float fTimeDelta)
{
...
_float4 CJar_4th::Get_NextStepPos()
{
...
void CJar_4th::Collision_Enter(COLLISION_DESC* pDesc, _uint iOtherObjectLayer, _float3 vContactPoint, _float3 ContactNormal, COLLISION_DESC* pMyDesc)
{
...
void CJar_4th::Collision_Stay(COLLISION_DESC* pDesc, _uint iOtherObjectLayer, _float3 vContactPoint, _float3 ContactNormal, COLLISION_DESC* pMyDesc)
{
    if (iOtherObjectLayer == ENUM_CLASS(COLLISION_LAYER::CAMERA) || iOtherObjectLayer == ENUM_CLASS(COLLISION_LAYER::MONSTER))
        return;
...
void CJar_4th::Collision_Exit(COLLISION_DESC* pDesc, _uint iOtherObjectLayer, COLLISION_DESC* pMyDesc)
{
    if (iOtherObjectLayer == ENUM_CLASS(COLLISION_LAYER::CAMERA) || iOtherObjectLayer == ENUM_CLASS(COLLISION_LAYER::MONSTER))
        return;
...
CJar_4th* CJar_4th::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
    CJar_4th* pInstance = new CJar_4th(pDevice, pContext);

    if (FAILED(pInstance->Initialize_Prototype()))
    {
        MSG_BOX(TEXT("Failed To Created : CJar_4th"));
...
CGameObject* CJar_4th::Clone(void* pArg)
{
    CJar_4th* pInstance = new CJar_4th(*this);

    if (FAILED(pInstance->Initialize_Clone(pArg)))
    {
        MSG_BOX(TEXT("Failed To Cloned : CJar_4th"));
        Safe_Release(pInstance);
    }
