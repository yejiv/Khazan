...
CChunk::CChunk(const CChunk& Prototype)
    : CGameObject{ Prototype }
{
}

HRESULT CChunk::Initialize_Prototype()
{
    return S_OK;
}

HRESULT CChunk::Initialize_Clone(void* pArg)
{
    ...
    CHUNK_DESC* pDesc = static_cast<CHUNK_DESC*>(pArg);
...
void CChunk::Priority_Update(_float fTimeDelta)
{
...
void CChunk::Late_Update(_float fTimeDelta)
{
...
void CChunk::Collision_Stay(COLLISION_DESC* pDesc, _uint iOtherObjectLayer, _float3 vContactPoint, _float3 ContactNormal, COLLISION_DESC* pMyDesc)
{

}

void CChunk::Collision_Exit(COLLISION_DESC* pDesc, _uint iOtherObjectLayer, COLLISION_DESC* pMyDesc)
{

}

void CChunk::Destory(_float3 vVelocity, _float3 vHitPos, _bool isHit, _float fLifeTime, _float fImpulse, _float fTorque)
{
...
void CChunk::Explode(_float3 vExplosionPos, _float fImpulse, _float fTorque, _float fLifeTime)
{
    m_isDestroyStart = true;
    m_isExplode = true;
...
HRESULT CChunk::Ready_Components(void* pArg)
{
    CHUNK_DESC* pDesc = static_cast<CHUNK_DESC*>(pArg);
    CHECK_NULLPTR(pDesc, E_FAIL);
    ...
    CHECK_FAILED(CGameObject::Add_Component(m_iLevelIndex , pDesc->strModelTag,
...
HRESULT CChunk::Ready_Collision(void* pArg)
{
...
HRESULT CChunk::Bind_ShaderResources()
{
    // 월드 행렬 쉐이더에 바인딩
...
HRESULT CChunk::Bind_Materials(_uint iMeshIndex)
{
    m_iMtrlFlags = 0;
...
HRESULT CChunk::Bind_DissolveValues()
{
...
CChunk* CChunk::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
    CChunk* pInstance = new CChunk(pDevice, pContext);

    if (FAILED(pInstance->Initialize_Prototype()))
    {
        MSG_BOX(TEXT("Failed to Created : CChunk"));
...
CGameObject* CChunk::Clone(void* pArg)
{
    CChunk* pInstance = new CChunk(*this);

    if (FAILED(pInstance->Initialize_Clone(pArg)))
    {
        MSG_BOX(TEXT("Failed To Cloned : CChunk"));
        Safe_Release(pInstance);
    }
