...
HRESULT CNPC_Duimuk::Render()
{
    ...
    for (_uint i = 0; i < iNumMeshes; ++i)
    {
        ...
        if (1 == i)
        {
            ...
            _float4 vEyeWhiteColor = { 3.5f, 3.5f, 3.5f, 1.f };
            // 홍채 외각
            _float4 vPupilCircle = { 0.35f, 0.25f, 0.1f, 1.f };
            // 홍채 중간
            _float4 vPupilRing = { 0.47f, 0.975f, 1.55f, 1.f };
            // 홍채 내각
            _float4 vPupilLens = { 0.25f, 0.15f, 0.06f, 1.f };

            // 크기 계열 (캐릭터 고정값)
            // 홍채 전체 크기
            _float fIrisRadius = 0.23f;
            // 동공 기본 크기
...
HRESULT CNPC_Duimuk::Ready_Collision(void* pArg)
{
...
HRESULT CNPC_Duimuk::Ready_Interaction_Guide(void* pArg)
{
...
HRESULT CNPC_Duimuk::Ready_OwnLight(void* pArg)
{
    ...
    m_wstrLightTag = TEXT("Duimuk_OwnLight");
...
void CNPC_Duimuk::Input_Interact_Event(_float fTimeDelta)
{
...
void CNPC_Duimuk::Animation_Update(_float fTimeDelta)
{
    if (false == m_isCollision)
        return;
...
void CNPC_Duimuk::Animation_Change(_float fTimeDelta)
{
    // NPC 가동 끝나면
