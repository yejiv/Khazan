...
HRESULT CVIBuffer_Mesh_Instance::Initialize_Prototype(INSTANCE_DESC* pArg)
{
	...
	_uint iNumMesh;
	MESH_INFO tMeshInfo = {};
	size_t iMeshNameLength;
	is.read(reinterpret_cast<char*>(&iNumMesh), sizeof(_uint));
	is.read(reinterpret_cast<_char*>(&tMeshInfo), sizeof(MESH_INFO));
	is.read(reinterpret_cast<char*>(&iMeshNameLength), sizeof(size_t));
	_char				name[MAX_PATH] = {};
	is.read(name, sizeof(_char) * iMeshNameLength);

	m_iNumInstance = pMeshDesc->iNumInstance;

	m_iNumIndexPerInstance = tMeshInfo.iNumFaces * 3;
	...
	for (_uint i = 0; i < m_iNumVertices; ++i)
	{
		...
		memcpy(&pVertices[i].vTranslation, &VtxData.vTranslation, sizeof(_float3));
		...
		m_pVertexPositions[i] = pVertices[i].vTranslation;
	}
...
CVIBuffer_Mesh_Instance* CVIBuffer_Mesh_Instance::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext, void* pDesc)
{
