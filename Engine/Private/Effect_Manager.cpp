...
void CEffect_Manager::Add_Effect_ToPool(_uint iLayerLevelIndex, const _wstring& strPrototypeTag, _uint iPoolSize)
{
    ...
    for (_uint i = 0; i < iPoolSize; ++i)
    {
        ...
        Safe_AddRef(effect);
        effect->SetID(ID);
        //effect->SetName(strPrototypeTag);
    }
}
...
_uint CEffect_Manager::Spawn_Effect(_uint iLayerLevelIndex, const _wstring& strPrototypeTag, _fvector SpwanPos)
{
    ...
    //m_pRunningEffects_Test.push_back(effect);
...
void CEffect_Manager::Priority_Update(_float fEffectTimeDelta)
{
    ...
    //for (auto& Pair : m_pRunningEffects_Test)
    //    Pair->Priority_Update(fEffectTimeDelta);
...
void CEffect_Manager::Update(_float fEffectTimeDelta)
{
    ...
    //for (_uint i = 0; i < m_pRunningEffects_Test.size(); )
    //{
    //    m_pRunningEffects_Test[i]->Update(fEffectTimeDelta);
    //
    //    if (m_pRunningEffects_Test[i]->IsActive() == false)
    //    {
    //        auto pool = Find_Effect_Pool(m_iCurLevel, m_pRunningEffects_Test[i]->GetName());
    //        pool->push_back(m_pRunningEffects_Test[i]);
    //        m_pRunningEffects_Test[i] = m_pRunningEffects_Test.back();
    //        m_pRunningEffects_Test.pop_back();
    //    }
    //    else
    //        ++i;
    //}
...
void CEffect_Manager::Late_Update(_float fEffectTimeDelta)
{
    ...
    //for (auto& Pair : m_pRunningEffects_Test)
    //    Pair->Late_Update(fEffectTimeDelta);
}
...
list<class CPrefab*>* CEffect_Manager::Find_RunningEffect_Layer(const _wstring& strLayerTag)
{
    ...
    //auto	iter = m_pRunningEffects_Test.find(strLayerTag);
    //
    //if (iter == m_pRunningEffects_Test.end())
    //{
    //    m_pRunningEffects_Test[strLayerTag];
    //    return Find_RunningEffect_Layer(strLayerTag);
    //}
