...
void CCSM::Update(_float fTimeDelta)
{
    ...
    for (_uint i = 0; i < m_Cascade.iNumCascades; ++i)
    {
        ...
        for (_uint j = 0; j < 4; ++j)
        {
            ...
            XMStoreFloat4(&m_FrustumCorners[j], vCornerNear);
        ...
        for (_uint j = 0; j < 8; ++j)
        {
            _float fDistance = XMVectorGetX(XMVector3Length(XMLoadFloat4(&m_FrustumCorners[j]) - vCenter));
        ...
        for (_uint j = 0; j < 8; ++j)
        {
            _vector vCornerWorld = XMLoadFloat4(&m_FrustumCorners[j]);
...
HRESULT CCSM::Bind_CascadeDSV(_uint iIndex)
{
...
HRESULT CCSM::Bind_Cascade_ShaderResources(CShader* pShader)
{
...
const _float4x4* CCSM::Get_CurrentLightViewMatrix() const
{
    return &m_Cascade.LightViewMatrices[m_iCurrentCascade];
}

const _float4x4* CCSM::Get_CurrentLightProjMatrix() const
{
    return &m_Cascade.LightProjMatrices[m_iCurrentCascade];
}

void CCSM::Set_CascadeConfig(CASCADE_CONFIG Config)
{
...
void CCSM::Clear_DSVs()
{
    for (_uint i = 0; i < m_Cascade.iNumCascades; ++i)
        m_pContext->ClearDepthStencilView(m_ShadowDSVs[i], D3D11_CLEAR_DEPTH, 1.f, 0);
}

void CCSM::Start_CascadeShadowTransition(_float fDuration, _float fTargetIntensity)
{
    m_isTransition = true;
    m_fTransTimeAcc = 0.f;
...
#ifdef _DEBUG
HRESULT CCSM::Ready_Debug(_float fX, _float fY, _float fSizeX, _float fSizeY)
{
...
HRESULT CCSM::Render(CShader* pShader, CVIBuffer_Rect* pVIBuffer)
{
    if (FAILED(pShader->Bind_SRV("g_TextureArray", m_pShadowSRVArray)))
        return E_FAIL;
...
HRESULT CCSM::Ready_ShaderResources()
{
...
CCSM* CCSM::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
    ...
    {
        MSG_BOX(TEXT("Failed to Create : CCSM"));
