...
#pragma region Animation
    ...
    typedef struct tagAnimationSetUp
    {
        ...
        bool                    isLoop = { false };
        unsigned int            iDirection = { 0 };                         // 방향 DIRECTION
        ...
        bool                    isIgnoreRootRot = { false };            // 회전 적용 여부
        bool                    isApplyRootPosition = { true };            // 위치 적용 여부
        bool                    isIgnoreRootPos = { false };                 // 매 프레임 루트본 위치값 0
        bool                    isIgnoreRootPosFirstFrame = { true };       //첫 번째 프레임만 루트본 위치값 0
        bool                    isAbsoluteRootPosition = { false };         //루트본이 절대 위치로 시작하는 애나메이션에 사용(오프셋 필요) (ex- 앞으로 가는 콤보공격 2,3번째같은경우)
