...
public:
	...
	bool CreateDeferredContexts(uint32_t count) {
		m_DeferredContexts.resize(count);
		for (uint32_t i = 0; i < count; ++i) {
			if (FAILED(m_pDevice->CreateDeferredContext(0, &m_DeferredContexts[i])))
				return false;
		}
		return true;
	}
	ID3D11DeviceContext* GetDeferredContext(uint32_t idx) const { return m_DeferredContexts[idx]; }
	_uint	GetDeferredContext_Count() { return m_DeferredContexts.size(); }
	ID3D11Device* GetDevice() const { return m_pDevice; }
	ID3D11DeviceContext* GetImmediate() const { return m_pDeviceContext; }
